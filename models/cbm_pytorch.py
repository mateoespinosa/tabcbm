...
def compute_bin_accuracy(c_pred, y_pred, c_true, y_true):
    ...
    for concept_idx in range(c_true.shape[-1]):
        selected_samples = torch.logical_not(torch.isnan(c_true[:, concept_idx]))
        if torch.any(selected_samples):
            seen_concepts += 1
            c_accuracy += sklearn.metrics.accuracy_score(
                torch.masked_select(c_true[:, concept_idx], selected_samples).float().cpu().detach(),
                torch.masked_select(c_pred[:, concept_idx], selected_samples).float().cpu().detach(),
            )
        ...
        c_auc = c_auc / seen_concepts
        c_f1 = c_f1 / seen_concepts
        c_accuracy = c_accuracy / seen_concepts
...
def compute_accuracy(
    ...
    for concept_idx in range(c_true.shape[-1]):
        ...
        if torch.any(selected_samples):
            seen_concepts += 1
            c_accuracy += sklearn.metrics.accuracy_score(
                c_true[selected_samples, concept_idx],
                c_pred[selected_samples, concept_idx],
            )
            try:
                c_auc += sklearn.metrics.roc_auc_score(
                    c_true[selected_samples, concept_idx],
                ...
                # If it so happens that there is only one class
                # for c_true and an error is thrown, then use
...
class ConceptBottleneckModel(pl.LightningModule):
    ...
    def _run_step(self, batch, batch_idx, train=False):
        ...
        if self.concept_loss_weight != 0:
            ...
            concept_loss = 0.0
            seen_concepts = 0
            if torch.any(torch.isnan(c_sem)):
                raise ValueError("NAN IN C_CEM!!!!")
            for aligned_idx, concept_idx in enumerate(range(c.shape[-1])):
                selected_samples = torch.logical_not(torch.isnan(c[:, concept_idx]))
                if torch.any(selected_samples):
                    seen_concepts += 1
                    concept_loss += self.loss_concept(
                        c_sem[selected_samples, concept_idx:concept_idx+1].float(),
                        c[selected_samples, aligned_idx:aligned_idx+1].float(),
                    )
            concept_loss = concept_loss / (seen_concepts + 1e-15)
