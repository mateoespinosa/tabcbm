'''
Modified from https://github.com/lucidrains/tab-transformer-pytorch. All credit
goes to the authors of "TabTransformer" Xin Huang, Ashish Khetan,
Milan Cvitkovic, and Zohar Karnin.
'''
import numpy as np
...
class TabTransformer(pl.LightningModule):
    ...
    def forward(self, x_input):
        concat_list = []
        if self.cat_idxs:
            x_categ = x_input[:, self.cat_idxs]
            x_categ = x_categ.type(torch.cuda.IntTensor)
            ...
            concat_list.append(flat_categ)
        if self.cont_idxs:
            x_cont = x_input[:, self.cont_idxs]
            ...
            concat_list.append(normed_cont)
