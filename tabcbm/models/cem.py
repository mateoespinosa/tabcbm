...
from tabcbm.models.cbm_pytorch import ConceptBottleneckModel
...
class ConceptEmbeddingModel(ConceptBottleneckModel):
    def __init__(
        ...
        try:
            ...
            num_latent_acts = list(
                self.pre_concept_model.modules()
            )[-1].out_features
        except:
            self.pre_concept_model = c_extractor_arch(
                output_dim=(n_latent_acts or n_concepts)
            )
    ...
    def _after_interventions(
        ...
        selected_samples = torch.logical_not(
            torch.isnan(c_true[:, concept_idx:concept_idx+1])
        )
